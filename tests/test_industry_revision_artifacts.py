...
def test_deck_order_and_profile_count() -> None:
    ...
    assert "Escopo, fontes e limitações" in slides[36]
    profiles = slides[37:57]
    ...
    assert "Ex-360 publicável" in slides[14]
    assert "R$ 6,89 bi" in slides[14]
    assert "4,4% da carteira" in slides[15]
    assert "13,5%" in slides[16]
    assert "9,1 p.p." in slides[16]
    assert "PRESTADORES · EVOLUÇÃO E RANKING" in slides[57]
    assert "FIDCs DOS CINCO BANCOS · COORTE ATUAL" in slides[58]
    assert "PRESTADORES · LIDERANÇA EXPLICADA" in slides[59]
    assert "MARKET SHARE · ADMINISTRAÇÃO" in slides[60]
    assert "MARKET SHARE · GESTÃO" in slides[61]
    assert "MARKET SHARE · CUSTÓDIA" in slides[62]
    assert "PRESTADORES · EVIDÊNCIAS DE MIGRAÇÃO" in slides[63]
    assert "Administração por subtipo" in slides[64]
    assert "Gestão por subtipo" in slides[65]
    assert "Custódia por subtipo" in slides[66]
    assert "APÊNDICE · CASO ATLÂNTICO" in slides[67]
    assert "09.194.841/0001-51" in slides[67]
    assert "A quebra no bruto coincide" in slides[67]
    deck_text = "\n".join(slides)
...
def test_structural_audit_corrections_are_materialized_in_the_deck() -> None:
    ...
    assert "70,1% do aumento líquido" in slides[13]
    assert "29,9% do Tipo analítico Outros" in slides[22]
    assert "Judicial/Precatórios/NPL" in slides[22]
    assert "FICs excluídos pelo portão único" in slides[27]
    assert "Kanastra permanece separada do Itaú" in slides[27]
    assert "sobre jan–jun/25" in slides[29]
    assert re.search(r"2022 FY.*N/D N/D", slides[29])
    assert slides[16].index("Setor público") < slides[16].index("Agronegócio")
    assert "66,0% dos R$ 77,7 bi" in slides[35]
    assert "PRESTADORES · EVIDÊNCIAS DE MIGRAÇÃO" in slides[63]
    assert "100,0% é coerente com a estratégia NPL" in slides[67]
...
def test_offer_slides_use_native_charts_and_editable_native_tables() -> None:
    _require(PPTX)
    with ZipFile(PPTX) as archive:
        ticket_charts = _slide_chart_paths(archive, 31)
        ticket_charts = [
        ...
        slide25 = ET.fromstring(archive.read("ppt/slides/slide31.xml"))
        ...
        regime_charts = _slide_chart_paths(archive, 32)
        regime_charts = [
            path for path in regime_charts
            if ET.fromstring(archive.read(path)).find(f".//{{{CHART}}}barChart") is not None
        ]
        assert len(regime_charts) == 4
        slide26 = ET.fromstring(archive.read("ppt/slides/slide32.xml"))
        assert slide26.findall(f".//{{{DML}}}tbl") == []

        assert _slide_chart_paths(archive, 33) == []
        slide27 = ET.fromstring(archive.read("ppt/slides/slide33.xml"))
        ...
        slide28 = ET.fromstring(archive.read("ppt/slides/slide34.xml"))
        assert len(slide28.findall(f".//{{{DML}}}tbl")) == 2
        slide29 = ET.fromstring(archive.read("ppt/slides/slide35.xml"))
...
@pytest.mark.parametrize("slide_number", [61, 62, 63, 65, 66, 67])
...
def test_provider_ranking_slide_has_six_native_charts_and_method_note() -> None:
    _require(PPTX)
    with ZipFile(PPTX) as archive:
        slide = ET.fromstring(archive.read("ppt/slides/slide58.xml"))
        text = " ".join(node.text or "" for node in slide.iter(f"{{{DML}}}t"))
        chart_paths = _slide_chart_paths(archive, 58)
...
def test_revision_renderer_version_tracks_export_simplification() -> None:
    ...
    assert 'const RENDERER_VERSION = "industry_revision_artifacts_v27";' in source
