...
def test_loads_and_reconciles_materialized_closed_offer_tables() -> None:
    ...
    assert len(tables.monthly) == 42
    assert tables.monthly.iloc[-1]["competence"] == "2026-07"
    assert not bool(tables.monthly.iloc[-1]["is_complete_month"])
    assert len(tables.originators) == 19
...
def test_full_payload_is_json_serializable_and_has_stable_blocks() -> None:
    ...
    assert payload["monthly"]["row_count"] == 42
    assert payload["jan_may_2024_2026"]["row_count"] == 3
    assert payload["originators_2026_ytd"]["row_count"] == 19
    assert payload["annual"]["source"]["archive_sha256"] == (
        "36eec864e5ade872457c935cf77b1d00e7540015c7f8240f11c9bde11cf15836"
    )


def test_jan_may_view_recalculates_comparable_ticket_and_volume() -> None:
    tables = load_closed_offers_tables()
    rows = build_jan_may_closed_offers_payload(tables.monthly)["rows"]
    by_year = {row["year"]: row for row in rows}

    expected = {
        2024: (345, 35_577_973_494.89, 103_124_560.85475363),
        2025: (581, 44_915_587_783.26, 77_307_380.00561102),
        2026: (554, 51_475_115_157.09, 92_915_370.31965703),
    }
    ...
    for year, (offers, volume, ticket) in expected.items():
        ...
        assert by_year[year]["period_end"] == f"{year}-05-31"
...
def test_nominable_originator_list_is_complete_sorted_and_reconciled() -> None:
    ...
    assert [row["rank"] for row in rows] == list(range(1, 20))
    ...
    assert math.isclose(identified, 22_046_288_000.0, abs_tol=0.01)
    assert math.isclose(rows[0]["identified_registered_volume_coverage"], 0.3168271557927403)
