...
def _build_export_dataframe(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    return export.rename(
        columns={
            ...
            "reinvestimento_excesso": "Reinvestimento de excesso de spread",
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "```",
            "",
            "- Em carteira revolvente, `carteira` é o saldo em aberto usado para juros e perda; ele se mantém reciclando o principal recebido (reinvestimento de principal) enquanto a nova carteira couber no prazo do FIDC. O excesso de spread após custos, perdas e PMTs de SEN/MEZZ também pode comprar carteira nova, mas só até a carteira atingir o PL inicial do FIDC (sem alavancagem implícita); o que excede esse limite vai para o caixa aplicado à SELIC (seção 10).",
            ...
            "- O excesso de spread após custos, perdas e PMTs de SEN/MEZZ (`fluxo_remanescente_apos_MEZZ`, quando positivo) também recicla em carteira nova enquanto a revolvência é elegível, mas só até a carteira atingir o PL inicial do FIDC. O que excede esse limite soma diretamente ao caixa aplicado à SELIC (seção 10).",
            "- O denominador principal de carteira originada usa a originação efetiva do motor, isto é, o principal reciclado mês a mês mais o eventual excesso de spread reciclado dentro do limite do PL inicial.",
            "",
            "```text",
            ...
            "reinvestimento_excesso_t = min(max(fluxo_remanescente_apos_MEZZ_t, 0), max(PL_FIDC_inicial - carteira_apos_principal_t, 0))",
            "```",
            "",
            "- No cenário sem perdas (usado para o colchão econômico), `principal_recebido_liquido` é igual a `principal_programado` em todo período elegível, a carteira já se mantém igual ao PL inicial e `reinvestimento_excesso_t = 0`; por isso `carteira_originada_efetiva` converge para o giro programático do volume inicial.",
            "- Em cenários com perdas, parte do principal programado vira inadimplência projetada e não é reciclada, e a baixa de crédito pode reduzir a carteira abaixo do PL inicial; nesse caso, parte do excesso de spread é reciclada para repor esse limite, e `carteira_originada_efetiva` reflete esse reinvestimento adicional.",
            ...
            "",
            "- Enquanto a revolvência é elegível, o modelo recicla o principal recebido líquido em novos recebíveis; o excesso de spread após custos, perdas e PMTs de SEN/MEZZ (`fluxo_remanescente_apos_MEZZ`, quando positivo) também recicla em carteira nova até a carteira atingir o PL inicial do FIDC. Só o que excede esse limite entra no saldo de caixa SELIC desde o primeiro período.",
            "- Quando o prazo médio dos recebíveis já não cabe mais no prazo restante do FIDC, o principal recebido também deixa de ser reciclado e passa a entrar no saldo de caixa SELIC; o excesso de spread deixa de reciclar em carteira e passa a entrar integralmente no caixa SELIC.",
            "- A taxa SELIC média é uma projeção digitada pelo usuário por ano calendário; nesta etapa ela não vem de fonte externa.",
            "- Esta curva manual remunera o caixa acumulado a cada período: o que sobra do excesso de spread depois de respeitado o limite do PL inicial e, depois do mês limite de reinvestimento, também o principal recebido.",
            ...
            "reinvestimento_excesso = min(max(fluxo_remanescente_apos_MEZZ, 0), max(PL_FIDC_inicial - carteira_apos_principal, 0))",
...
def _build_step_by_step_markdown() -> str:
    return "\n".join(
        [
            ...
            "- **Excesso de spread** é o fluxo positivo remanescente após custos, perdas e pagamentos de SEN/MEZZ. Ele recicla em carteira nova até a carteira atingir o PL inicial do FIDC (sem alavancagem implícita); o que excede esse limite vai para o caixa aplicado à SELIC.",
            ...
            "- **Carteira originada efetiva** é a base reconciliada com o motor: volume inicial + soma do principal reciclado mês a mês + soma do excesso de spread reciclado, quando a carteira cai abaixo do PL inicial e há espaço para repô-la.",
            "- No cenário sem perdas, as duas bases convergem: o motor recicla exatamente o principal programado e a carteira já se mantém igual ao PL inicial, sem espaço para reinvestir excesso. Em cenários com perdas, a baixa de crédito pode abrir esse espaço; parte do excesso de spread entra então na carteira originada efetiva para repor o PL inicial, enquanto o principal que virou inadimplência projetada continua fora da reciclagem.",
    ...
    memory_df = pd.DataFrame(
        [
            ...
            },
            {
                ...
                "Observação": "Em carteira revolvente, a base de carteira evolui com o principal reciclado (reinvestimento de principal) enquanto houver prazo para nova originação, mais o eventual excesso de spread reciclado quando a carteira está abaixo do PL inicial do FIDC.",
            },
            {
                ...
                "Observação": "O caixa SELIC recebe, desde o primeiro mês, a parcela do excesso de fluxo após custos, perdas e PMTs de SEN/MEZZ (quando positivo) que não coube na carteira por causa do limite do PL inicial; quando a carteira não pode mais comprar recebíveis, o principal que vence também passa a compor esse caixa e a render pela SELIC média anual informada pelo usuário.",
            },
            {
                ...
                "Observação": "Este é o fluxo usado antes de custos, provisão/perda e pagamentos das cotas; a SELIC remunera o saldo de caixa acumulado, que recebe a parcela do excesso de spread que excedeu o limite do PL inicial e, fora da janela de reinvestimento, também o principal recebido.",
            },
            {
            ...
            },
            {
                ...
                "Fórmula": "se elegível: principal_recebido_líquido + reinvestimento_excesso; se não elegível: 0",
                "Observação": "reinvestimento_excesso = min(max(fluxo_remanescente_apos_MEZZ, 0), max(PL_FIDC_inicial - carteira_apos_principal, 0)); captura o principal reciclado mais o excesso de spread reciclado dentro do limite do PL inicial. Fora da janela elegível, principal recebido e excesso de spread vão para o caixa SELIC.",
            },
            {
            ...
            },
            {
                ...
                "Observação": "Denominador reconciliado com o motor; inclui o reinvestimento de principal reciclado e o eventual excesso de spread reciclado (capado pelo PL inicial) enquanto a revolvência é elegível.",
            },
            {
