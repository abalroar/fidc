...
A inadimplência já vem no `vehicle_monthly` que a carteira lê. A provisão não:
ela mora na Tabela I do Informe Mensal, no campo *redução ao valor recuperável*
(`TAB_I2A11_VL_REDUCAO_RECUP` e o par sem coobrigação `TAB_I2B11_*`), e a PDD do
fundo é a soma dos dois blocos.

O join é por **`(cnpj, competencia)`**, não só por CNPJ. A carteira mistura meses
de propósito — cada fundo entra com o seu mais recente —, e casar a PDD de junho
com a inadimplência de março inventaria um quociente que nunca existiu.

A leitura é uma só: **PDD ÷ inadimplência**, com o limiar em 100%. Abaixo dele o
fundo provisionou menos do que já venceu, e a barra fica vermelha; daí para cima,
verde-azulada. O par vale ΔE 14,9 em deuteranopia, e a linha tracejada dos 100%
reforça a leitura sem depender da cor.

### O que não tem denominador

| estado | o que significa |
| --- | --- |
| `com cobertura` | tem inadimplência; entra no gráfico |
| `sem inadimplência` | tem carteira, nada em atraso — não existe quociente |
| `sem carteira` | sem carteira de direitos creditórios na competência |

Um fundo sem atraso **não** é um fundo sem cobertura. Ele sai do gráfico e é
contado na legenda de rodapé, em vez de virar uma barra em zero.

### O teto de 200%

A distribuição é dominada por denominadores minúsculos: há fundo com R$ 30 mil de
PDD sobre R$ 121 de inadimplência, o que dá 24.884%. Cobertura de 300% e de
24.000% dizem a mesma coisa — amplamente provisionado —, e a segunda esmaga todas
as outras barras contra o chão.

A barra para em `TETO_PCT`; **o rótulo em cima traz o número verdadeiro**. É por
isso que o rótulo vai em todas as barras, e não só nas notáveis: ele é o que
devolve o que o teto corta.
